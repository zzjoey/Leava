...
var _goTo = function(detail){
    ...
    if(!_url) return;
    if(!_type) {
        if (detail._execDetail.redirect) wx.redirectTo({url:_url});
        else wx.navigateTo({url:_url});
        return;  
    } 
    switch (_type){
        case "redirect" :  wx.redirectTo({url:_url});break;
        case "navigate" :  wx.navigateTo({url:_url});break;
        case "nagivator" :  wx.navigateTo({url:_url});break;  //兼容，后续可以去掉
        default : wx.switchTab({url:_url}); break;
    }
}  
//显示/隐藏元素 class
